   ...
   public async Task<IEnumerable<Product>> GetByCompanyIdListAsync(Guid companyId)
{
    return await _context.Products
        ...
        .Include(p => p.Brand)
        .Include(p => p.Company)
        .Where(p => !p.IsDeleted && p.CompanyId == companyId)  // ✅ Filtreyi (p.CompanyId == companyId) doğrudan veritabanı sorgusuna (SQL) gömdüğün için, veriler API'ye gelmeden önce filtrelenmiş oldu. Bu en güvenli yaklaşımdır.
        .ToListAsync();
}
